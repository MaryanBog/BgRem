...
class RangeSeekBarView @JvmOverloads constructor(
    ...
    private fun pixelToScale(index: Int, pixelValue: Float): Float {
        val scale = pixelValue * PIXEL_100 / mPixelRangeMax
        return if (index == 0) {
            val pxThumb = scale * mThumbWidth / PIXEL_100
            scale + pxThumb * PIXEL_100 / mPixelRangeMax
        } else {
            val pxThumb = (PIXEL_100 - scale) * mThumbWidth / PIXEL_100
            scale - pxThumb * PIXEL_100 / mPixelRangeMax
        }
    }

    private fun scaleToPixel(index: Int, scaleValue: Float): Float {
        val px = scaleValue * mPixelRangeMax / PIXEL_100
        return if (index == 0) {
            val pxThumb = scaleValue * mThumbWidth / PIXEL_100
            px - pxThumb
        } else {
            val pxThumb = (PIXEL_100 - scaleValue) * mThumbWidth / PIXEL_100
            px + pxThumb
        }
    }
    ...
    companion object{
        private const val PIXEL_100 = 100
    }
}
